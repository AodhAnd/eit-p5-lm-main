...
%csvwrite('simulationPForwardsingleStep.csv', ScopeData3);
simData = csvread('simulationPForwardsingleStep.csv', 0, 0);
simData(:,1) = simData(:,1)-2

data = csvread('LOG00322.csv'); % step 294 for calculated PI controller
                                % step 288 for tuned PI controller
                                % step 299 for P controller with feed forward
                                % step 316 (without) and 322 (width) anti-windup

%data = csvread('LOG00217.csv');%, 0, 0); 18 to 22    use step 16
data(:,1) = (data(:,1)-3980)/1000;
...
set(AX(2),'xlim',[0 3], 'ylim', [1.5 8], 'ycolor', [.7 0 0]);
...
title('PI-Controller at 2 m \cdot s^{-1}')
xlabel('Time [s]')
